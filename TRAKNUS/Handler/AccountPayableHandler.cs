...
namespace TRAKNUS.Handler
{
    public class AccountPayableHandler
    {
        public List<AccountPayableModel> readText(System.Web.HttpContextBase context)
        {
            ...
            string fileName = "D:/Kerja/test.txt";
            ...
            foreach (var car in baca)
            {
                ...
                if (!string.IsNullOrEmpty(car))
                {
                    ...
                    AP.document_number = Convert.ToInt32(car.Split('|')[1]);
                    AP.fiscal_year = Convert.ToInt32(car.Split('|')[2]);
                    AP.number_of_line_item = Convert.ToInt32(car.Split('|')[3]);
                    AP.document_date = Convert.ToDateTime(car.Split('|')[4]);
                    AP.posting_date = Convert.ToDateTime(car.Split('|')[5]);
                    ...
                    AP.exchange_rate = Convert.ToInt32(car.Split('|')[11]);
                    AP.transaction_date = Convert.ToDateTime(car.Split('|')[12]);
                    ...
                    //skip 2
                    AP.gl_account_number = Convert.ToInt32(car.Split('|')[19]);
                    AP.amount_in_document = float.Parse(car.Split('|')[20], CultureInfo.InvariantCulture.NumberFormat);
                    AP.amount_in_local_currency = Convert.ToInt32(car.Split('|')[21]);
                    AP.tax_code = car.Split('|')[22];
                    AP.business_area = car.Split('|')[23];
                    //skip 2
                    AP.value_date = Convert.ToDateTime(car.Split('|')[26]);
                    AP.term_of_payment = car.Split('|')[27];
                    AP.day = Convert.ToInt32(car.Split('|')[28]);
                    AP.assigment = car.Split('|')[29];
                    AP.keterangan = car.Split('|')[30];
                    AP.invoice_reference_dan_tahun = Convert.ToInt32(car.Split('|')[31]);
                    AP.payment_reference = Convert.ToInt32(car.Split('|')[32]);
                    //skip
